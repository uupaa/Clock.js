...
|-----------------------------|------|------|--------|-----|
| webkitRequestAnimationFrame | 6.0+ | 4.4+ | YES    | 10+ |
| requestAnimationFrame       | 7.0+ | 4.4+ | YES    | 10+ |
| performance.now()           | 8.0+ | 4.4+ | YES    | 10+ |
 */
...
var RAF              = global["requestAnimationFrame"] ||       // iOS 7.0+
                       global["webkitRequestAnimationFrame"];   // iOS 6.0+
var RAF_X            = global["cancelAnimationFrame"] ||        // iOS 7.0+
                       global["webkitCancelAnimationFrame"];    // iOS 6.0+
...
//}@dev
    ...
    this._lastTimeStamp = -1;                   // Number - last time stamp.
    ...
    function _onsuspend(hiddenState) {
        if (hiddenState) {
            that._suspendTime.begin = Date.now();
            that["stop"]();
        } else {
            that._suspendTime.total += ( Date.now() - that._suspendTime.begin );
            that._suspendTime.begin = 0;
            that["start"]();
        }
    }
}
...
Clock["prototype"] = Object.create(Clock, {
    ...
    // --- utility ---
    "resetMode":    { "value":  Clock_resetMode     }, // Clock#resetMode(options:Object - { vsync, pulse, wait }):void
    ...
    // --- accessor ---
    "vsync": {
        "set":  function(v) { this["resetMode"]({ "vsync": v }); },
        "get":  function()  { return this._mode; }
    },
    "pulse": {
        "set":  function(v) { this["resetMode"]({ "pulse": v }); },
        "get":  function()  { return this._pulse; }
    },
    "wait": {
        "set":  function(v) { this["resetMode"]({ "wait": v }); },
...
function _enterFrame(timeStamp) { // @arg DOMHighResTimeStamp|undefined - requestAnimationFrame give us timeStamp.
    ...
    case MODE_RAF:         this._timerID = RAF(this._enterFrame);
    }
    ...
    if (timeStamp < this._suspendTime.total) {
        // [!] reset susped time
        this._suspendTime.total = timeStamp;
    }
    // reduce the suspended time
    timeStamp -= this._suspendTime.total;
    ...
    this._lastTimeStamp = timeStamp; // update time stamp
...
function Clock_stop() { // @ret this
    ...
    if (this._active) {
        ...
        case MODE_RAF:         RAF_X(this._timerID);
        }
        this._timerID = 0;
    }
    return this;
...
function Clock_resetMode(options) { // @arg Object - { vsync, wait, pulse }
                                    // @options.vsync Boolean = current.vsync
                                    // @options.wait  Number  = current.wait
                                    // @options.pulse Number  = current.pulse
//{@dev
    if (!global["BENCHMARK"]) {
        $valid($type(options, "Object"), Clock_resetMode, "options");
        $valid($keys(options, "vsync|wait|pulse"),   Clock_resetMode, "options");
        $valid($type(options.vsync, "Boolean|omit"), Clock_resetMode, "options.vsync");
        $valid($type(options.wait,  "Number|omit"),  Clock_resetMode, "options.wait");
        $valid($type(options.pulse, "Number|omit"),  Clock_resetMode, "options.pulse");
    }
//}@dev
    ...
    if ("vsync" in options) {
        this._mode = options["vsync"] ? RAF_SWITCH : MODE_INTERVAL;
    }
    if ("wait" in options) {
        this._wait = options["wait"];
    }
    if ("pulse" in options) {
        this._pulse = options["pulse"];
    }
    if (active) {
        this["start"]();
    }
